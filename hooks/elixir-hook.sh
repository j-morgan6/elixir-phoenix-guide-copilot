...
if [ -z "${OSTYPE:-}" ] && [ -n "${COMSPEC:-}" ]; then
  PS1_SCRIPT="$SCRIPT_DIR/elixir-hook.ps1"
  if [ -f "$PS1_SCRIPT" ] && command -v powershell.exe > /dev/null 2>&1; then
    exec powershell.exe -ExecutionPolicy Bypass -File "$PS1_SCRIPT" "$PHASE" "$TYPE"
  fi
  exit 0
fi
...
check_bash_pre() {
  ...
  if echo "$cmd" | grep -qE 'mix\s+ecto\.reset'; then
    echo 'Dangerous: mix ecto.reset destroys and recreates the database. Use mix ecto.rollback for safe rollbacks.' >&2
    exit 2
  fi

  # Hook 2: Block force push
  if echo "$cmd" | grep -qE 'git\s+push\s+.*(-f|--force)'; then
    echo 'Dangerous: Force push overwrites remote history. Use --force-with-lease for safer force pushes.' >&2
    exit 2
  fi

  # Hook 3: Block MIX_ENV=prod
  if echo "$cmd" | grep -qE 'MIX_ENV=prod'; then
    echo 'Dangerous: Running with MIX_ENV=prod locally risks unintended production effects.' >&2
    exit 2
  fi
...
check_edit_pre() {
  ...
  if [ "$HAS_LV" != 'false' ]; then
    if echo "$filtered" | grep -qE 'def\s+(mount|handle_event|handle_info|handle_call|handle_cast|render|init|terminate)\(' \
      ...
      echo 'Missing @impl true before callback function. Add @impl true on the line before each callback.' >&2
      exit 2
    fi
  fi
  ...
  if grep -qE '(upload_path|file_path|uploads_dir)\s*=\s*["'"'"'](/|priv/)' <<< "$content" 2>/dev/null; then
    echo 'Hardcoded file path detected. Move to Application config: config :my_app, :upload_path, "priv/static/uploads"' >&2
    exit 2
  fi
  ...
  if grep -qE '(max_file_size|file_size_limit|max_upload|max_size)\s*=\s*[0-9]{7,}' <<< "$content" 2>/dev/null; then
    echo 'Hardcoded file size limit detected (7+ digits). Move to Application config.' >&2
    exit 2
  fi
  ...
  if [ "$is_test" = "false" ] && [ "$ext" != "heex" ]; then
    if grep -qE 'String\.to_atom\(' <<< "$content" 2>/dev/null; then
      echo 'String.to_atom/1 detected — atom table exhaustion risk! Use a whitelist or keep as strings.' >&2
      exit 2
    fi
  fi
  ...
  if [ "$is_test" = "false" ] && [ "$ext" != "heex" ]; then
    if grep -qE 'fragment\(".*#\{' <<< "$content" 2>/dev/null; then
      echo 'String interpolation inside Ecto fragment — SQL injection risk! Use parameterized fragments with ? placeholders.' >&2
      exit 2
    fi
  fi
  ...
  if [ "$is_test" = "false" ] && [ "$ext" != "heex" ]; then
    if grep -qE 'redirect\(.*to:\s*(params|conn\.params|socket\.assigns)\[' <<< "$content" 2>/dev/null; then
      echo 'Open redirect detected — redirecting to user-controlled URL! Validate against a whitelist or use verified routes.' >&2
      exit 2
    fi
  fi
  ...
  if grep -qE '<\.(flash_group|flash)' <<< "$content" 2>/dev/null; then
    echo '.flash_group is deprecated in Phoenix 1.8+. Flash handling is automatic in layouts.' >&2
    exit 2
  fi
  if grep -qE 'form_for\(' <<< "$content" 2>/dev/null; then
    echo 'form_for is deprecated. Use <.form for={to_form(@changeset)}> instead.' >&2
    exit 2
  fi
  if grep -qE 'live_redirect|live_patch' <<< "$content" 2>/dev/null; then
    echo 'live_redirect/live_patch are deprecated. Use <.link navigate={path}> or push_navigate/push_patch.' >&2
    exit 2
  fi
  if [ "$HAS_SCOPE" = 'true' ] && grep -qE '@current_user|current_user' <<< "$content" 2>/dev/null \
    ...
    echo 'Phoenix 1.8+ detected: Use @current_scope instead of @current_user. Access user via @current_scope.user' >&2
    exit 2
  fi
  ...
  if grep -qE 'def static_paths' <<< "$content" 2>/dev/null; then
    ...
    for ref in $refs; do
      if ! echo " $paths " | grep -qw "$ref"; then
        echo "Path reference '/$ref/' found but '$ref' not in static_paths(). Add it to your static_paths definition." >&2
        exit 2
      fi
    done
  fi
  ...
  if [ "$is_test" = "false" ] && [ "$ext" != "heex" ]; then
    if grep -qE 'Ecto\.Adapters\.SQL\.(query|query!)|SQL\.query' <<< "$content" 2>/dev/null; then
      if echo "$content" | grep -qE '".*#\{.*\}.*"|".*\$\{.*\}.*"' || echo "$content" | grep -qE 'query.*<>|<>.*query'; then
        echo 'Raw SQL with string interpolation detected — SQL injection risk! Use parameterized queries ($1, $2, ...).' >&2
        exit 2
      fi
    fi
  fi
